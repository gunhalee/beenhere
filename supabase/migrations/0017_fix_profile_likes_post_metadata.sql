-- Profile likes metadata fix (2026-04-10)
-- Split original-post metadata from like-time metadata so UI can render both correctly.
...
RETURNS TABLE (
  ...
  like_distance_meters double precision,
  like_count bigint,
  my_like boolean
)
LANGUAGE plpgsql
...
BEGIN
  ...
  SELECT
    ...
    like_loc.place_label AS like_place_label,
    CASE
      WHEN viewer_lat IS NULL
        OR viewer_lng IS NULL
        OR like_loc.latitude IS NULL
        OR like_loc.longitude IS NULL
      THEN NULL
      ELSE (
        6371000.0 * acos(
          LEAST(
